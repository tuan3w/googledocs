...
goo.Bitmap.extractFeature = function(input, maxHeight){
    ...
    //upperCase data : if uppercase set last row with 0.5 else -0.6
   // alert(height + ', ' + maxHeight)
    var uVal;
  //  if (uVal  == - .5) alert('-.05')
  	if (height < 0.2 * maxHeight) 
  		uVal = 0;
   ...
   // alert("height" + height + "," +cellWidth);
    ...
    for(var row = 0, i= 0; row<DOWNSAMPLE_WIDTH; row++ )
    {
        for(var col = 0; col<DOWNSAMPLE_WIDTH; col++)
        {
           ...
           // alert( gray + ',' + out[row][col] + '');
       
            // if( d ){
            //     out[row][col] = 1;  //back point
            // } else {
            //     out[row][col] = 0;  //white point
            // }
        }
    }
    ...
    for (var i=0; i< DOWNSAMPLE_WIDTH; i++) {
    	...
    	var gray = 0;
    	for (var j=DOWNSAMPLE_WIDTH/2; j< DOWNSAMPLE_WIDTH; j++) 
    		gray += out[j][i];
    	out[DOWNSAMPLE_WIDTH][i] = gray * 2.0 / (DOWNSAMPLE_HEIGHT);

    	// var gray = 0;
    	// for (var j=DOWNSAMPLE_WIDTH/2 + 1; j< DOWNSAMPLE_WIDTH; j++) 
    	// 	gray += out[j][i];
    	// out[DOWNSAMPLE_WIDTH + 1][i] = gray * 1.0 / (DOWNSAMPLE_HEIGHT/2);

    	gray = 0;
    	for (var j=0; j< DOWNSAMPLE_WIDTH; j++)
    		gray += out[i][j];
    	out[DOWNSAMPLE_WIDTH + 1][i] = gray * 2.0 / DOWNSAMPLE_WIDTH;
    	if ( gray == 0) {
    		out[DOWNSAMPLE_WIDTH + 2][i] = 0;
    		//alert('i')
    	}else
    		out[DOWNSAMPLE_WIDTH + 2][i] = 0.5;

    	//alert( gray * 1.0 / (255 * height))
    	out[DOWNSAMPLE_WIDTH + 3][i] = uVal;
    }
  // alert(out)
    return out;
};
...
goo.OCRTool = function(containerId,onload){
	...
	this.text = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ(),.'; //--> todo
};
